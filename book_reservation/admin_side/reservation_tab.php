...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['reservation_id'], $_POST['action'])) {
    ...
    exit; // Ensure we exit after JSON response
}
...
<body>
    <!-- ADDED wrapper so main content leaves space for the sidebar to open on hover. This mirrors book_summary.php: ml-[15rem] gives the left margin for the sidebar. -->
    <div id="main-content" class="flex-1 flex flex-col min-w-0 ml-[15rem] h-screen transition-all duration-300 p-6 bg-gray-100">
        ...
        <table>
            ...
            <tbody>
                <?php foreach ($reservations as $row): $now = new DateTime(); $expiry = new DateTime($row['expiry_time']); $statusClass = $row['status']; ?>
                        ...
                        <div class="action-container" id="container-<?= $row['reservation_id'] ?>">
                            ...
                            <div class="overlay-prompt" id="prompt-<?= $row['reservation_id'] ?>">
                                <div class="prompt-content">
                                    <div class="prompt-text"> Mark this reservation as <strong id="prompt-action-text-<?= $row['reservation_id'] ?>"></strong>? </div>
