...
def ask(question: str) -> dict:
    ...
    exact_hits = df.index[df["TITLE"].str.strip().str.lower()
    ...
    seed_vec      = embedder.encode(top_title + " " + top_summary,
                                    convert_to_tensor=True)
    related_hits  = util.semantic_search(seed_vec, corpus_embeddings, top_k=5)[0]

    related = []
    for hit in related_hits:
        if hit["corpus_id"] == seed_idx or hit["score"] < 0.30 \
           or len(related) >= 2:
            continue
        ...
        # 🔒 Skip if summary too short or title looks like a placeholder
        if len(summary_r) < 50:
            continue
        ...
        rel_short = client.chat.completions.create(
            ...
            messages=[{
                "role": "user",
                "content": (f'Summarize this book titled "{title_r}" '
                            f'in 2‑3 sentences:\n\n{summary_r}')
            }],
            max_tokens=80,
            temperature=0.4,
